...
func ListReviewFilm(db *gorm.DB) error {
	jsonFile, err := os.Open("migrations/json/review.json")
	if err != nil {
		return err 
	}
	...
	var listReviewFilm []entity.Review
	if err := json.Unmarshal(jsonData, &listReviewFilm); err != nil {
		return err 
	}

	hasTable := db.Migrator().HasTable(&entity.Review{})
	if !hasTable {
		if err := db.Migrator().CreateTable(&entity.Review{}); err != nil {
			return err
		}
	}
	...
	for _, data := range listReviewFilm {
		...
		err := db.Where(&entity.Review{UserID: data.UserID, FilmID: data.FilmID}).First(&existingReview).Error
